...
export function registerOneOnOneWs(app: FastifyInstance, redisUrl: string) {
  ...
  app.get(
    ...
    async (socket, request) => {
      ...
      // Authenticate via signed token (generated by POST /:id/ws-token)
      const token = query.token;
      if (!token) {
        sendJson(socket, { type: "error", message: "Missing auth token — use POST /one-on-one-sessions/:id/ws-token first" });
        socket.close();
        return;
      }
