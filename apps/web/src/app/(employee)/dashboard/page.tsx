...
async function TopRow({
  ...
  return (
    <div className="mb-8 grid gap-6 lg:grid-cols-12">
      ...
      >
        <EngagementRing score={currentScore} />
        <div className="mt-4 flex items-center gap-2">
          <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-medium ${
            scoreDelta >= 0 ? "bg-positive/10 text-positive" : "bg-danger/10 text-danger"
          }`}>
...
async function ChartsRow({
  ...
  try {
    ...
    const [engResult, fbResult, orgResult] = await Promise.allSettled([
      getEngagementScoresForUser(db, userId),
      getFeedbackForSubject(db, userId),
    ...
    if (engResult.status === "fulfilled" && engResult.value.length > 0) {
      const scores = engResult.value;
      engagementHistory = scores.slice(0, 6).reverse().map((s) => ({
...
async function SessionsSection({
  ...
  let oneOnOneSessions: SessionRow[] = isDemo
    ? (mockOneOnOneSessions as unknown as SessionRow[])
    : [];
  let hasManager = isDemo;

  try {
    const db = getDb();
    const [meResult, ooResult] = await Promise.allSettled([
      getUserWithManager(db, userId),
      getSessionsForPair(db, userId),
    ]);
    ...
    if (!hasManager) {
      ...
      oneOnOneSessions = ooResult.value;
    }
  } catch {
    // use defaults
  }
...
async function FeedbackSection({
  ...
  try {
    ...
    if (fbResult.status === "fulfilled" && fbResult.value.length > 0) {
      const entries = fbResult.value.slice(0, 3);
