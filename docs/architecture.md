...
Key tables per tenant DB: `users`, `user_platform_identities`, `teams`, `core_values`, `conversations`, `conversation_messages`, `feedback_entries`, `feedback_value_scores`, `kudos`, `engagement_scores`, `escalations`, `escalation_audit_log`, `questionnaires`, `questionnaire_themes`, `interaction_schedule`, `pulse_check_triggers`, `calendar_events`
...
- User/org/team models + auth (NextAuth.js with SSO)
- Conversation orchestrator (multi-turn state machine)
- AI pipeline: LLM gateway + question generation from questionnaire themes
- Feedback storage + analysis (sentiment, engagement scoring)
- Interaction scheduler (BullMQ cron)
- Neo4j setup + manual relationship connections
- Wire dashboards to real API data

### Phase 3: Intelligence (Weeks 9-12)
- Calendar sync (Google Calendar, Outlook)
- Auto-relationship graph from calendar data
